// src/routes/auth.js
import { Router } from 'express';
import { OAuth2Client } from 'google-auth-library';
import jwt from 'jsonwebtoken';
import cfg from '../config/env.js';
import User from '../models/User.js';

const router = Router();
const googleClient = new OAuth2Client(cfg.googleClientId);

function issueCookie(res, payload) {
  const token = jwt.sign(payload, cfg.jwtSecret, { expiresIn: '7d' });
  res.cookie(cfg.cookieName, token, {
    httpOnly: true,
    sameSite: 'lax',      
    secure: false,        
    maxAge: 7 * 24 * 60 * 60 * 1000,
  });
}

/**
 * POST /api/auth/google
 * Body: { idToken }
 * Verifies the Google ID token, upserts a user, sets our cookie.
 */
router.post('/auth/google', async (req, res) => {
  try {
    const { idToken } = req.body || {};
    if (!idToken) return res.status(400).json({ error: 'idToken required' });

    // --- Debug: show which client issued the token vs what backend expects
    try {
      const preview = JSON.parse(Buffer.from(idToken.split('.')[1], 'base64').toString('utf8'));
      console.log('[AUTH] ID token aud:', preview.aud, 'expected:', cfg.googleClientId);
      console.log('[AUTH] ID token iss:', preview.iss, 'email:', preview.email);
    } catch {
      console.log('[AUTH] (could not preview ID token payload)');
    }

    // Verify with Google
    const ticket = await googleClient.verifyIdToken({
      idToken,
      audience: cfg.googleClientId, // must exactly match Web Client ID
    });
    const payload = ticket.getPayload(); // verified
    // payload: { sub, email, name, picture, aud, iss, exp, ... }

    if (!payload?.email || !payload?.sub) {
      return res.status(401).json({ error: 'invalid_google_payload' });
    }

    // Upsert user
    let user = await User.findOne({ googleId: payload.sub });
    if (!user) {
      user = await User.create({
        googleId: payload.sub,
        email: payload.email,
        name: payload.name,
        picture: payload.picture,
      });
    } else {
      // keep profile fresh (optional)
      const changed = (user.name !== payload.name) || (user.picture !== payload.picture);
      if (changed) {
        user.name = payload.name;
        user.picture = payload.picture;
        await user.save();
      }
    }

    // Issue our cookie
    issueCookie(res, { id: user._id.toString(), email: user.email, name: user.name });

    return res.json({
      ok: true,
      user: { id: user._id, email: user.email, name: user.name, picture: user.picture },
    });
  } catch (e) {
    console.error('[AUTH] Google verification failed:', e?.message || e);
    return res.status(401).json({ error: 'google_verification_failed', detail: e?.message || 'verifyIdToken error' });
  }
});

/**
 * GET /api/auth/me
 * Returns the current user payload from our cookie (or null).
 * This route should be PUBLIC (do not wrap with requireAuth).
 */
router.get('/auth/me', (req, res) => {
  const raw = req.cookies?.[cfg.cookieName];
  if (!raw) return res.json({ user: null });
  try {
    const u = jwt.verify(raw, cfg.jwtSecret); // { id, email, name, iat, exp }
    return res.json({ user: { id: u.id, email: u.email, name: u.name } });
  } catch {
    return res.json({ user: null });
  }
});

/**
 * POST /api/auth/logout
 * Clears our session cookie.
 */
router.post('/auth/logout', (req, res) => {
  res.clearCookie(cfg.cookieName, {
    httpOnly: true,
    sameSite: 'lax',
    secure: false, // true in production HTTPS
  });
  return res.json({ ok: true });
});

export default router;
